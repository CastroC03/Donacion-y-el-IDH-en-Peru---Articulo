...
ggplot(wss_df, aes(x = k, y = wss)) +
  geom_line(color = "#a7c6ed", size = 2.5) +
...
# 4. Convertir a objeto dendrograma y asignar colores (5 clusters con paleta Obake Ito)
dend <- as.dendrogram(hc)
dend_colored <- color_branches(dend, k = 5, 
                                col = c("#f0b8aa", "#b4e0d1", "#f2d0a7", "#d8b4e2", "#a7c6ed"))

# 5. Personalizar etiquetas y grosor de líneas
labels_colors(dend_colored) <- "black"               # Color de etiquetas
labels_cex(dend_colored) <- 0.7                      # Tamaño del texto
dend_colored <- set(dend_colored, "branches_lwd", 2.5)  # Grosor de líneas
labels(dend_colored) <- datos_ancho$Departamento     # Etiquetas con nombres

# 6. Graficar
plot(dend_colored,
     main = "Dendrograma de Clustering Jerárquico (k = 5)",
     xlab = "",
     sub = "",
     ylab = "Distancia Euclidiana",
     cex.main = 1.2)

mtext("Fuente: Elaboración propia", side = 1, line = 4, adj = 1, cex = 0.8)
```
...
# 2. Cálculo de silueta para varios k
sil_plot <- fviz_nbclust(
  ...
  k.max = 7,
  nstart = 25,
  diss = dist(datos_cluster, method = "euclidean")
)

# 3. Extraer datos del gráfico
sil_data <- sil_plot$data

# 4. Volver a graficar con ggplot para personalización completa
ggplot(sil_data, aes(x = clusters, y = y, group = 1)) +
  geom_line(color = "#a7c6ed", size = 1.5) +          # Línea de unión
  geom_point(color = "red", size = 3) +             # Puntos grandes y rojos
  geom_vline(xintercept = sil_data$clusters[which.max(sil_data$y)],
             linetype = "dashed", color = "#a7c6ed", size = 1) +  # Línea punteada negra y gruesa
  labs(
    title = "Método de Silueta",
    caption = "Fuente: Elaboración propia",
    x = "Número de Clusters (k)",
...
```{r}
set.seed(123)
kmeans_resultado <- kmeans(datos_cluster, centers = 5, nstart = 25)
...
```{r}
limpieza_datos_ancho1 <- c("2" = "6","4" = "7","5" = "8","3" = "9","1" = "10")
limpieza_datos_ancho2 <- c("6" = "Muy Alto", "7" = "Alto", "8" = "Medio", "9" = "Bajo", "10" = "Muy Bajo")
...
pca_df$cluster <- factor(pca_df$cluster, levels = c("Muy Alto", "Alto", "Medio", "Bajo", "Muy Bajo"))
...
ggplot(pca_df, aes(x = PC1, y = PC2, color = cluster, label = Departamento)) +
  ...
  labs(
    title = "Gráfico PCA con 5 clusters",
    caption = "Fuente: Elaboración propia",
    x = "Componente Principal 1",
    y = "Componente Principal 2",
    color = "Cluster"
  ) +
  scale_color_manual(values = c("Muy Alto"="#f0b8aa",
                                "Alto"="#d8b4e2",
                                "Medio"="#f2d0a7",
                                "Bajo"="#b4e0d1",
                                "Muy Bajo"="#a7c6ed")) +
...
ggplot(pca_df, aes(x = PC1, y = PC2, color = cluster)) +
  ...
  scale_color_manual(values = c("Muy Alto"="#a7c6ed",
                                "Alto"="#d8b4e2",
                                "Medio"="#f2d0a7",
                                "Bajo"="#b4e0d1",
                                "Muy Bajo"="#f0b8aa")) +
  scale_fill_manual(values = c("Muy Alto"="#a7c6ed",
                               "Alto"="#d8b4e2",
                               "Medio"="#f2d0a7",
                               "Bajo"="#b4e0d1",
                               "Muy Bajo"="#f0b8aa")) +
...
PeruClusters$cluster <- factor(PeruClusters$cluster, 
                               levels = c("Muy Alto", "Alto", "Medio", "Bajo", "Muy Bajo"))

ggplot(PeruClusters) +
  geom_sf(aes(fill = cluster), color = "white", size = 0.2) +
  labs(
    title = "Agrupación de Departamentos por grupo Cluster",
    subtitle = "Resultado del análisis de clusters K-means (k = 5)",
    caption = "Fuente: Elaboración propia",
    x = "Longitud", y = "Latitud",
    fill = "Cluster"
  ) +
  scale_fill_manual(values = c("Muy Alto" = "#f0b8aa",
                               "Alto"     = "#d8b4e2",
                               "Medio"    = "#f2d0a7",
                               "Bajo"     = "#b4e0d1",
                               "Muy Bajo" = "#a7c6ed")) +
...
# Asignar niveles de cluster para 5 grupos
IDH_clusters$cluster <- factor(IDH_clusters$cluster,
...
ggplot(IDH_clusters, aes(x = IDH, y = NOMBDEP, fill = cluster)) +
  ...
  labs(
    title = "IDH por Departamento y Grupo Cluster",
    caption = "Fuente: Elaboración propia",
...
```{r}
promedio_proporcion <- datos_ancho %>%
  rowwise() %>%
  mutate(promedio = mean(c_across(starts_with("T")), na.rm = TRUE)) %>%
  ungroup() %>%
  select(NOMBDEP, promedio)
```

```{r}
cv_promprop <- sd(promedio_proporcion$promedio) / mean(promedio_proporcion$promedio) * 100
cv_promprop
```


```{r}
sf.test(promedio_proporcion$promedio) 
shapiro.test(promedio_proporcion$promedio)
lillie.test(promedio_proporcion$promedio)
```

Como en las tres pruebas de normalidad tenemos un valor de p mayor a 0.05, no rechazamos la hipótesis nula y concluimos que la variable tiene una distribución promedio tiene un distribución normal

```{r}
ggplot(datos_ancho, aes(x = factor(cluster), y = promedio_proporcion$promedio)) +
  geom_boxplot() +
  labs(x = "Cluster", y = "Proporción promedio de donantes")
```

```{r}
datos_ancho %>%
  group_by(cluster) %>%
  summarise(p_value = shapiro.test(promedio_proporcion$promedio)$p.value)
```

Dado que el valor p es mayor que 0.05, no se rechaza la hipótesis nula de normalidad.
Esto sugiere que la distribución de la proporción promedio de donantes dentro de los 5 clusters no difiere significativamente de una distribución normal. Asumimos normalidad

Evaluación de normalidad para IDH por Departamento

```{r}
sf.test(IDH_data$IDH) 
shapiro.test(IDH_data$IDH)
lillie.test(IDH_data$IDH)
```
Dado que el valor p es mayor que 0.05 en todas las pruebas, no se rechaza la hipótesis nula de normalidad. Esto sugiere que la variable IDH por departamento tiene un distribución normal.


Evaluación de normalidad para la proporción promedio de donantes por Departamento
...
lillie.test(promedio_proporcion$promedio)
```
Dado que el valor p es mayor que 0.05 en todas las pruebas, no se rechaza la hipótesis nula de normalidad. Esto sugiere que la variable proporción promedio de donantes por departamento tiene un distribución normal.


#### Análisis de correlación

Como se concluyó una distribución NORMAL. Debemos aplicar la prueba de correlación de Pearson
...
El análisis de correlación entre el Índice de Desarrollo Humano (IDH) y la proporción promedio de donantes por departamento revela una asociación estadísticamente significativa. La prueba de correlación de Pearson, que evalúa relaciones lineales, arrojó un valor de p = 7.258e-10. Este resultado se encuentra muy por debajo del umbral común de significancia (α = 0.05), lo que indica que existe una asociación significativa entre el nivel de desarrollo humano y la disposición a donar órganos. 
